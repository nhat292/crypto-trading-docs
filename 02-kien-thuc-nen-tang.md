...
## ⚖️ Spot vs Margin vs Futures/Perpetual

```mermaid
flowchart LR
    A["🟢 Spot\nSở hữu coin thật"] --> B["🟡 Margin\nVay thêm vốn"] --> C["🔴 Futures/Perpetual\nĐòn bẩy cao, phái sinh"]

    classDef low fill:#10b981,stroke:#065f46,color:#fff
    classDef mid fill:#f59e0b,stroke:#92400e,color:#fff
    classDef high fill:#ef4444,stroke:#991b1b,color:#fff
    class A low
    class B mid
    class C high
```

*Thang rủi ro tăng dần từ trái sang phải.*
...
> [!TIP]
> **Lời khuyên cho người mới**: bắt đầu và thành thạo ở Spot trước. Chỉ
> chuyển sang Futures/Margin khi đã có kỷ luật quản trị rủi ro vững (xem
> [🛡️ 05-quan-tri-rui-ro.md](05-quan-tri-rui-ro.md)) và hiểu rõ cơ chế
> liquidation bên dưới.
...
- **Liquidation (Thanh lý)**: khi lỗ chạm đến mức ký quỹ tối thiểu, sàn tự
  động đóng vị thế và mất toàn bộ phần ký quỹ đã bỏ vào lệnh đó.

> [!CAUTION]
> Đây là lý do các cây "râu nến" quét mạnh hay xảy ra quanh vùng thanh khoản
> dày đặc lệnh đòn bẩy (xem [🐋 09-doc-vi-ca-map-onchain.md](09-doc-vi-ca-map-onchain.md)).
...
> [!WARNING]
> Không để toàn bộ vốn trên sàn giao dịch. Bật 2FA (ưu tiên ứng dụng xác
> thực hơn SMS), không bao giờ chia sẻ seed phrase/private key với bất kỳ
> ai, cảnh giác với link giả mạo (phishing) giả danh sàn/dự án.
