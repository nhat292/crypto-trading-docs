...
```mermaid
flowchart TD
    PPI["🏭 PPI tăng\n(chi phí nhà máy)"] --> CPI["📈 CPI/PCE tăng\n(giá bán lẻ)"]
    CPI --> INF["🔥 Lạm phát tăng"]
    INF --> FED["🏦 Fed tăng lãi suất"]
    FED --> DXY["💵 DXY & US10Y tăng"]
    DXY --> CRYPTO["🔴 Crypto SẬP"]

    NFP["👷 NFP tăng mạnh\n(nhiều việc làm)"] --> INC["💰 Thu nhập & chi tiêu tăng"]
    INC --> INF

    classDef bad fill:#ef4444,stroke:#991b1b,color:#fff
    classDef neutral fill:#3b82f6,stroke:#1e40af,color:#fff
    class CRYPTO bad
    class PPI,CPI,INF,FED,DXY,NFP,INC neutral
```
